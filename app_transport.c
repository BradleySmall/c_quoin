 ...
 *
 * There is an arbitrary array of Passes done so simply for example. There is no testing of it
 * overrunning its boundaries. The storage would be better done in a dynamic memory structure like
 * a list or tree, or even better a database. 
 *
 *
 */
#include <stdio.h>
...
void Buy_A_Pass(void)
{
	...
	/* get user type */
	entry = Menu_Prompt_Char("What type of user?", "(1) Standard, (2) Student, (3) Elderly, (0) Worker", '0', '3');
	switch ( entry ) {
		case '0':
	...
	if ( user != USERTYPE_WORKER) {
		entry = 'a';
		entry = Menu_Prompt_Char("What type of term?", "(1) Monthly, (2) Per Ride", '1', '2');
		switch ( entry ) {
			case '1':
				...
				/* because we have the day of month now, store it and pass it into the rate calculation
				 * instead of recalculating it later. 
				 *
				 * However, I would think all of the date calculations should be moved away from here.
				 */
		...
		entry = 'a';
		entry = Menu_Prompt_Char("What type of trans?", "(1) Bus, (2) Subway, (3) Rail, (4) Elderly Bus", '1', '4');
		switch ( entry ) {
			case '1':
				trans = TRANSTYPE_BUS;
				break;
		...
		/* when shortcircuiting for the worker, the term and trans are set to special values here */
	...
	for(i = 0; i < passIndex; ++i) {
		if (passes[i]->id == id) {
			...
			if (Pass_Use(passes[i], trans)) {
				...
				/* Failure 
				 *
				 * Rather than bools I could be returning an int that shows -1 for NSF and -2 for invalid transit
				 * etc. That may or may not be an improvement, but I could them return a more specific message
				 * in the event of the card being rejected.
				 * */
				puts("This pass may not have a high enough balance, or it may not be valid for this transit!\n");
			}
		}
	}
	/* however, if the pass is not in the system, do give that message specifically */
	if (!found) {
 ...
 *
 * it is thin, but satisfies the specification. One could dump all of the available info
 * on the pass at this point, but that was not asked for. 
 */
